...
REGISTRY_ALIASES=$(kubectl get cm registry-aliases -n kube-system -oyaml )
REGISTRY_SVC=$(kubectl get cm registry-aliases -n kube-system -oyaml ) 
ALIASES_ENTRIES=""
NL_DELIMITER='~'
SPACES='  '
...
kubectl get cm coredns -n kube-system -oyaml   | tee /tmp/coredns-alias-prepatch.yaml > /dev/null
...
then
   ...
   sed '/health/a\\ $ALIASES_ENTRIES' < /tmp/coredns-alias-prepatch.yaml| tr '~' '\n' | tee /tmp/coredns-alias-patch.yaml > /dev/null
